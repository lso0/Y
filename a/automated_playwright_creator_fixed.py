    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def handle_request(request):
                ...
                # Only show important requests to reduce noise
                if 'jmap' in request.url or 'auth' in request.url:
            ...
            # If we haven't captured a token yet, try to use the working script's approach
            if not bearer_token:
                print("⚠️  No JMAP API calls detected in headless mode.")
                print("💡 Falling back to the working version's approach: using the API token from .env")
                
                # Since headless mode doesn't trigger JMAP calls, use the API token directly
                api_token = os.getenv('FASTMAIL_API_TOKEN')
                if api_token:
                    bearer_token = api_token
                    print(f"✅ Using API token from .env: {bearer_token[:20]}...")
                    
                    # Extract user ID from the current URL if possible
                    current_url = page.url
                    if 'u=' in current_url:
                        user_id = current_url.split('u=')[1].split('&')[0].split('#')[0]
                        print(f"✅ Extracted User ID from URL: {user_id}")
                    else:
                        user_id = "2ef64041"  # Fallback from recording session
...
def create_alias_api(bearer_token, user_id, account_id, cookies, alias_email, target_email, description=""):
    ...
    # Use minimal capabilities that work with API tokens
    payload = {
        "using": [
        ...
        "methodCalls": [
            [
                "Identity/set",  # Use standard JMAP Identity/set method
                {
